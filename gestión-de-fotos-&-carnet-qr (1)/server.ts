    ...
    if (list.length > 0) {
      list.forEach(s => {
        ...
        const badgeStatus = s.estado === 'Presente'
          ? `<span class="badge-status-presente"><i class="bi bi-check-circle-fill me-1"></i> Presente</span>${horaText}`
          : `<span class="badge-status-pendiente"><i class="bi bi-clock me-1"></i> Pendiente</span>`;
      ...
      if (html.includes('<!-- STUDENTS_TABLE_BODY -->')) {
        html = html.replace('<!-- STUDENTS_TABLE_BODY -->', rowsHtml);
      } else {
      ...
      `;
      if (html.includes('<!-- STUDENTS_TABLE_BODY -->')) {
        html = html.replace('<!-- STUDENTS_TABLE_BODY -->', emptyHtml);
      } else {
